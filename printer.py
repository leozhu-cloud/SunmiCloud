...
class SunmiCloudPrinter:
    ...
    def httpPost(self, path: str, body: Dict[str, Any]) -> str:
        ...
        print('Leo', json.loads(response.text))
        print('leo_type', type(json.loads(response.text)))
    ...
    def onlineStatus(self, sn: str):
        body: Dict[str, Any] = {}
        body['sn'] = sn
        body['page_no'] = 1
        body['page_size'] = 3
        print('hello_test', body)
        printer_info = self.httpPost('/v2/printer/open/open/device/onlineStatus', body)

        sn_list = printer_info['data']['list']
        for p in sn_list:
            if p['is_online'] == 0:
                pass
            else:
    ...
    def newTicketNotify(self, sn: str) -> None:
        ...
        print('printer get a new order ticket')
...
if __name__ == '__main__':
    ...
    print('onlinestatus')
    sn = printer.onlineStatus('')
    print(sn)
    print('\n')

    trade_no = '{}_{:010d}'.format(sn, int(time.time()))

    printer.pushVoice(sn=sn, cycle=2, content='你有新订单', interval=2, expire_in = 300)

    # Order Information (directory from MDM or redirectory from Client's server)
    ...
    printer.appendText('*** hello hello  ***\n')
    ...
    printer.printInColumns(('Item', 'Amount\n(单位：随意)', 'Total\n($)(￥)(S$)'))
    printer.lineFeed()
    printer.printInColumns(('Beef', 'x3', '$20.99'))
    printer.lineFeed()
    printer.printInColumns(('Pork', 'x2', '$1.90'))
    printer.lineFeed()
    ...
    print('pushContent')
    printer.pushContent(trade_no=trade_no, sn=sn, count=1, media_text='hello pay attention')
    print('\n')
    ...
    print('newTicketNotify')
    printer.newTicketNotify(sn)
    print('\n')
